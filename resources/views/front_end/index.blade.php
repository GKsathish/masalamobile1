...
<img src="{{asset('assets/front_end/images/adimage/bottom2.jpg')}}"  alt="image11	" style="padding:30px; display:flex; justify-content:center; align-items:center" id="myDiv"/>
<!--second-->
<div onload="caller();">
...
-->    
    <!--video ad added by praveen-->
		...
		@php 
		    ...
		    $entertainmentinfo= DB::select(DB::raw('SELECT  * FROM(SELECT  imagepath,postid,posttitle,categoryid   FROM post  WHERE trending_now="TRENDING NOW" and language="English"   ORDER BY  published_date DESC LIMIT 15) AS temptable  LIMIT 12'));
			$entertainmentinfo1= DB::select(DB::raw('SELECT * FROM (SELECT videopath,postid,posttitle FROM paparazzi_post  WHERE trending_now="TRENDING NOW" and language="English"   ORDER BY published_date DESC LIMIT 8) AS temptable  LIMIT 4'));
      ...
      <div class="hashtag" style="padding: 10px;">
          <marquee behavior="scroll" direction="left"  width="100%">
			 	...
			 	$tag = DB::select(DB::raw('SELECT hashtag,count(*) as COUNT FROM `hashtags` WHERE  hashtag !="" and date(publisheddate)>=date(NOW()-INTERVAL 10 DAY) GROUP by hashtag HAVING COUNT(hashtag)>3 limit 16 offset 0;'));
				@endphp
	...
	<h2 class="left"><span><a href="/Horoscope_Details">Horoscope</a></span></h2>
		
	
		<div >
			<div class="visual-stories-carousel owl-carousel owl-theme clear">	
			@php
			$get_vs1 = DB::table('H_stories')->where('cat_type','horoscope')->orderBy('storyid','desc')->get();
			@endphp			
	...
	<section class="container equal-height padding-top-none clear">
    	<aside class="content left">
            <div class="slider-carousel owl-carousel owl-theme clear" id="slider-carousel-enter">
				@php 
					$entertainmentinfo = DB::table('post')->where('categoryid',8)->where('language','English')->where('status','Publish')->take(8)->orderBy('published_date','desc')->get(); 
				@endphp
				@foreach($entertainmentinfo as $entertainmentinfo1)
				
				<?php 
				$titlorl = $entertainmentinfo1->posttitle_new;
				$title = str_replace(" ","+",$titlorl);
				?>
        ...
        <aside class="sidebar right" style="margin:0;">
			...
			@php 
					$entertainmentinfo = DB::table('post')->where('categoryid',9)->where('language','English')->where('status','Publish')->take(4)->orderBy('published_date','desc')->get(); 
				@endphp
				@foreach($entertainmentinfo as $entertainmentinfo)
				<?php 
				$titlorl = $entertainmentinfo->posttitle;
	...
	<section class="container padding-top-none clear">
    	<aside class="content left">
            ...
            <div class="single-article clear">
            	<ul>
				@php 
				$entertainmentinfo = DB::table('post')->where('categoryid',2)->where('language','English')->where('status','Publish')->orderBy('published_date','desc')->take(3)->get(); 
				@endphp
				@foreach($entertainmentinfo as $entertainmentinfo)
						<?php 
				$titlorl = $entertainmentinfo->posttitle;
	...
	<div class="vid-main-wrapper clearfix" >
        ...
        <div class="vid-container" id = "videostories">
          @php
          $entertainmentinfo = DB::table('paparazzi_post')->where('language','English')->where('status','Publish')->where('language','Telugu')->where('cat_type','Video_Stories')->orderBy('postid','desc')->take(1)->get();
          @endphp
          @foreach($entertainmentinfo as $entertainmentinfo)
        ...
        <div class="vid-list-container" id="vid-list">
          <ol id="vid-list">
            @php
            $entertainmentinfo = DB::table('paparazzi_post')->where('language','English')->where('status','Publish')->where('cat_type','Video_Stories')->orderBy('postid','desc')->inRandomOrder()->take(20)->get();
            @endphp
	...
	<section class="container random-posts padding-top-none clear">
		...
		<div id="randomposts">
			<ul class="clear">
				@php 
				$entertainmentinfo = DB::table('post')->where('language','English')->where('status','Publish')->whereRaw('DATE(published_date) >= DATE_SUB(CURDATE(), INTERVAL 3 DAY)')->inRandomOrder()->take(5)->orderBy('published_date','desc')->get(); 
				@endphp
				@foreach($entertainmentinfo as $entertainmentinfo)
				<li>
	...
	<section class="container padding-top-none clear">
    	<aside class="content left">
            ...
            <div class="grid clear">
					@php 
					$entertainmentinfo = DB::table('post')->where('language','English')->where('categoryid',4)->where('status','Publish')->orderBy('published_date','desc')->take(3)->get(); 
                    @endphp
                    @foreach($entertainmentinfo as $entertainmentinfo)
                    <?php 
				$titlorl = $entertainmentinfo->posttitle;
	...
	<div class="vid-main-wrapper clearfix">
        ...
        <div class="vid-container" id = "paparazzi-section">
          @php
          $entertainmentinfo = DB::table('paparazzi_post')->where('language','English')->where('status','Publish')->where('cat_type','paparazzi')->orderBy('postid','desc')->take(1)->get();
          @endphp
          @foreach($entertainmentinfo as $entertainmentinfo)
        ...
        <div class="vid-list-container" id="vid-list">
          <ol id="vid-list">
            @php
            $entertainmentinfo = DB::table('paparazzi_post')->where('language','English')->where('status','Publish')->where('cat_type','<>','Video_Stories')->where('cat_type','paparazzi')->orderBy('postid','desc')->inRandomOrder()->take(20)->get();
            @endphp
          	@foreach($entertainmentinfo ->slice(1, 20) as $entertainmentinfo)
	...
	<section class="container random-posts padding-top-none clear">
		...
		<div id="randomposts">
			<ul class="clear">
				@php 
				$entertainmentinfo = DB::table('post')->where('categoryid',1)->where('language','English')->where('status','Publish')->orderBy('published_date','desc')->inRandomOrder()->take(10)->get(); 
				@endphp
				@foreach($entertainmentinfo as $entertainmentinfo)
				<li>
				<button style="float:right; position: relative;cursor: pointer; border: none;" class="open" value="{{$entertainmentinfo->postid}}"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-share-fill" viewBox="0 0 16 16">
	...
	<section class="container padding-top-none clear">
    	<aside class="content left">
            ...
            <div class="single-article clear">
            	<ul>
				@php 
			$entertainmentinfo = DB::table('post')->where('categoryid',7)->where('language','English')->where('status','Publish')->orderBy('published_date','desc')->take(6)->get(); 
			@endphp
			@foreach($entertainmentinfo as $entertainmentinfo)
			       <?php 
				$titlorl = $entertainmentinfo->posttitle;
	...
	<section class="container padding-top-none clear">
		...
		<div class="single-article-carousel owl-carousel owl-theme clear">
			@php 
			$entertainmentinfo = DB::table('post')->where('categoryid',6)->where('language','English')->where('status','Publish')->take(8)->orderBy('published_date','desc')->get(); 
			@endphp
			@foreach($entertainmentinfo as $entertainmentinfo)
			   <?php 
				$titlorl = $entertainmentinfo->posttitle;
	...
	<section class="container padding-top-none clear">
        ...
        <div class="tumbnailcarousel owl-carousel owl-theme clear">
			@php 
			$entertainmentinfo = DB::table('post')->where('categoryid',9)->where('language','English')->where('status','Publish')->orderBy('published_date','desc')->take(8)->get(); 
			@endphp
			@foreach($entertainmentinfo as $entertainmentinfo)
			 <?php 
				$titlorl = $entertainmentinfo->posttitle;
	...
	<section class="container padding-top-none clear">
		...
		<div class="single-article-carousel owl-carousel owl-theme clear">
			@php 
			$entertainmentinfo = DB::table('post')->where('categoryid',5)->where('language','English')->where('status','Publish')->take(8)->orderBy('published_date','desc')->get(); 
			@endphp
			@foreach($entertainmentinfo as $entertainmentinfo)
			 <?php 
				$titlorl = $entertainmentinfo->posttitle;
	...
	<!--Travel & Tourism-->
		...
		<div id="randomposts">
			<ul class="clear">
				@php 
				$entertainmentinfo = DB::table('post')->where('categoryid',11)->where('language','English')->where('status','Publish')->orderBy('published_date','desc')->inRandomOrder()->take(5)->get(); 
				@endphp
				@foreach($entertainmentinfo as $entertainmentinfo)
		...
		<section class="container equal-height padding-top-none clear">
			...
			<aside class="content left">
    			...
    			<article class="single-article-box clear">
    					@php 
					$entertainmentinfo = DB::table('post')->where('categoryid', 10)->where('language','English')->where('status','Publish')->orderBy('published_date','desc')->take(4)->get(); 
					@endphp
					@foreach($entertainmentinfo as $entertainmentinfo)
						 <?php 
				$titlorl = $entertainmentinfo->posttitle;
			...
			<aside class="sidebar right">
				<div class="news-list sidebarposts nomargin">
					...
					<div id="Recent" class="tabcontent clear">
						<ul>
						@php 
						$entertainmentinfo = DB::table('post')->where('status','Publish')->where('language','English')->orderBy('published_date','desc')->take(8)->get(); 
						@endphp
						@foreach($entertainmentinfo as $entertainmentinfo)
									 <?php 
				$titlorl = $entertainmentinfo->posttitle;
				...
				?>
					...
					<div id="Popular" class="tabcontent clear">
						...
						@php 
							$entertainmentinfo = DB::table('post')->where('language','English')->whereRaw('DATE(published_date) >= DATE_SUB(CURDATE(), INTERVAL 6 DAY)')->where('hitcount', '>',0)->where('status','Publish')->take(7)->orderBy('hitcount','desc')->get();
							@endphp
							@foreach($entertainmentinfo as $entertainmentinfo)
													 <?php 
				$titlorl = $entertainmentinfo->posttitle;
	...
	<section class="container padding-top-none clear">
        ...
        <div class="tumbnailcarousel owl-carousel owl-theme clear">
			@php 
			$entertainmentinfo = DB::table('post')->where('categoryid',17)->where('language','English')->where('status','Publish')->orderBy('published_date','desc')->take(8)->get(); 
			@endphp
			@foreach($entertainmentinfo as $entertainmentinfo)
													 <?php 
				$titlorl = $entertainmentinfo->posttitle;
