...
@endif
            ...
            <figure>
                <img src="{{$feed->videopath}}" alt="" width="100%">
                <iframe width="100%" height="700" src="{{$feed->videopath}}" id="mob_paparazzi_main" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
            </figure>
            ...
            <?php } ?>
                <div class="socialshare clear">
                    <ul>
                       <li><a href="http://www.facebook.com/sharer.php?u=<?php echo env('SITE_URL'); ?>paparazzi-post/<?php echo $feed->postlink; ?>" onClick="javascript: window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=500,width=500,padding=200');return false;" target="_blank"><i class="fab fa-facebook-f"></i></a></li>
                        <li> <a href="https://twitter.com/share?url=<?php echo env('SITE_URL'); ?>paparazzi-post&id=<?php echo $feed->postlink; ?>" onClick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=600');return false;" target="_blank"> <i class="fab fa-twitter"></i></a></li>
                        <!-- <li><a href="#"><i class="fa fa-pinterest"></i></a></li> -->
                        <li><a href="https://www.instagram.com/?url=<?php echo env('SITE_URL'); ?>paparazzi-post&id=<?php echo $feed->postlink; ?>" onClick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=600');return false;" target="_blank"><i class="fab fa-instagram"></i></a></li>
                        <!-- <li><a href="#"><i class="fa fa-linkedin"></i></a></li> -->
                       <li><a href="https://api.whatsapp.com/send?text=<?php echo env('SITE_URL'); ?><?php echo "paparazzi-post%26id=".$feed->postlink; ?>" data-action="share/whatsapp/share"  data-action="share/whatsapp/share" onClick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=600');return false;" target="_blank"><i class="fab fa-whatsapp"></i></a></li>
                    </ul>
                </div>
    			<script async src="https://securepubads.g.doubleclick.net/tag/js/gpt.js"></script>
        ...
        <div class="next-prev-button clear">
			...
			<a href="paparazzi-post&id={{$previous_feed->postlink}}" rel="prev">Previous</a>
            @endforeach
            @foreach ($next_feed as $next_feed)
            <a href="paparazzi-post&id={{$next_feed->postlink}}" rel="next">Next</a>
            @endforeach
		</div>
    </div>	
    @endforeach 
		 ...
		 <div class="sidebarposts clear">
                      ...
                      @php 
                        ...
                        @foreach($entertainmentinfo as $entertainmentinfo)
                            <li>
                            <a href="paparazzi-post&id={{$entertainmentinfo->postlink}}">
	    ...
	    <div class="ads">
			...
			<div class="sidebarposts clear">
				...
				<div id="Recent" class="tabcontent clear">
					<ul>
                        <?php 
                            // $entertainmentinfo = DB::table('paparazzi_post')->where('status','Publish')->inRandomOrder()->orderBy('published_date','desc')->take(10)->get(); 
                        ?> 
                    @php 
					$entertainmentinfo = DB::table('paparazzi_post')->where('status','Publish')->orderBy('published_date','desc')->skip(4)->take(10)->get(); 
                    @endphp
                    @foreach($entertainmentinfo as $entertainmentinfo)
						<li>
						<a href="paparazzi-post&id={{$entertainmentinfo->postlink}}">
				...
				<div id="Popular" class="tabcontent clear">
					  <ul>
                        @php 
                        $entertainmentinfo = DB::table('paparazzi_post')->whereRaw('DATE(published_date) >= DATE_SUB(CURDATE(), INTERVAL 6 DAY)')->where('hitcount', '>',0)->where('status','Publish')->take(10)->orderBy('hitcount','desc')->get();
                        @endphp
                        @foreach($entertainmentinfo as $entertainmentinfo)
                            <li>
                            <a href="paparazzi-post&id={{$entertainmentinfo->postlink}}">
